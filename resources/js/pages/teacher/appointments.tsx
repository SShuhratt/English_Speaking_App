...
export default function Appointments() {
    ...
    const handleModalActionSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (modalAction.type === 'cancel') {
                ...
                await axios.post(
                    `/teacher/appointments/${modalAction.id}/reject`,
    ...
    const statusStyle = (status: string) => {
        ...
        if (status === 'pending')
            return 'border-[#fae18e] bg-[#fffbeb] text-[#92700a]';
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <div className="flex items-start gap-4">
                                                <div className="flex h-11 w-11 shrink-0 items-center justify-center overflow-hidden rounded-xl bg-[#d0e4ff]/40 text-[#061445]">
                                                    {apt.pupil?.avatar ? (
                                                        <img
                                                            src={
                                                                apt.pupil.avatar
                                            ...
                                            <div className="flex flex-wrap items-center gap-2 md:shrink-0">
                                                {isPast ? (
                                                    <>
                                                        <span
                                                            className={`rounded-xl border px-3 py-1 text-[10px] font-black tracking-wider uppercase ${statusStyle(apt.status)}`}
                                                        >
                                                            {t(
                        ...
                        <p className="mt-2 text-sm text-slate-500">
                            {modalAction.type === 'cancel'
                                ? t('teacher.cancel_desc') ||
                                  'Please state the reason for cancelling this appointment.'
                                : t('teacher.reject_desc') ||
                                  'Please state the reason for rejecting this booking request.'}
                        </p>
                        <form
                            onSubmit={handleModalActionSubmit}
                            className="mt-4 space-y-4"
                        >
                            <div>
                                <textarea
                                    className="min-h-[100px] w-full rounded-xl border border-slate-200 bg-[#f8f9fc] px-3 py-2 text-sm focus:border-[#061445] focus:ring-2 focus:ring-[#061445]/10 focus:outline-none"
                                    placeholder={
